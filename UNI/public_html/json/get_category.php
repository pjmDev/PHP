...
*/
	...
	$db = new mDB();
	$strSQL = "select * from tb_category where 1=1 " . $whereStr;
	$strSQL.= "order by canum ";
	...
	while($rs->next()){
		...
		array_push($result, array('canum'=>$rs->f("canum"), 'cabcate'=>$rs->f("cabcate"), 'camcate'=>$rs->f("camcate"), 'cascate'=>$rs->f("cascate"), 
										'castandard'=>$rs->f("castandard")));
	}
